 ...
 * @p0_rx_aeq: IO mapped register base address of port0 Rx AEQ training
 * @p1_rx_aeq: IO mapped register base address of port1 Rx AEQ training
 */
struct airoha_pcie_phy {
	...
	void __iomem *p0_rx_aeq;
	void __iomem *p1_rx_aeq;
};
...
static int airoha_pcie_phy_init(struct phy *phy)
{
	...
	writel(val, pcie_phy->p0_rx_aeq + REG_PCIE_PEXTP_DIG_LN_RX30);
	writel(val, pcie_phy->p1_rx_aeq + REG_PCIE_PEXTP_DIG_LN_RX30);
...
static int airoha_pcie_phy_probe(struct platform_device *pdev)
{
	...
	pcie_phy->p0_rx_aeq =
		devm_platform_ioremap_resource_byname(pdev, "p0-rx-aeq");
	if (IS_ERR(pcie_phy->p0_rx_aeq))
		return dev_err_probe(dev, PTR_ERR(pcie_phy->p0_rx_aeq),
				     "Failed to map P0 Rx AEQ base\n");

	pcie_phy->p1_rx_aeq =
		devm_platform_ioremap_resource_byname(pdev, "p1-rx-aeq");
	if (IS_ERR(pcie_phy->p1_rx_aeq))
		return dev_err_probe(dev, PTR_ERR(pcie_phy->p1_rx_aeq),
				     "Failed to map P1 Rx AEQ base\n");
